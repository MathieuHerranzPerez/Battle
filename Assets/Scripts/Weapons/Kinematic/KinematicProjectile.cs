...
public abstract class KinematicProjectile : Projectile
{
	...
	[SerializeField] protected bool useCharge = false;
	[SerializeField] protected float maxStartSpeed = 80f;
	...
	[SerializeField]
	private AnimationCurve _interpolationEase;
	...
	public override ProjectileData GetFireData(NetworkRunner runner, Vector3 firePosition, Vector3 fireDirection, float charge)
	{
		float speed = useCharge ? Mathf.Lerp(_startSpeed, maxStartSpeed, charge) : _startSpeed;

        if (_maxLiveTimeTicks < 0)
		{
			int maxDistanceTicks = Mathf.RoundToInt((_maxDistance / speed) * runner.Simulation.Config.TickRate);
		...
		return new ProjectileData()
		{
			...
			FireVelocity = fireDirection * speed,
		};
	}
