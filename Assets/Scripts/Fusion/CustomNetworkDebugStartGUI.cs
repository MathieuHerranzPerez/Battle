...
/// <summary>
/// Companion component for <see cref="CustomNetworkDebugStart"/>. Automatically added as needed for rendering in-game networking IMGUI.
/// </summary>
[RequireComponent(typeof(CustomNetworkDebugStart))]
[AddComponentMenu("Fusion/Network Debug Start GUI")]
[ScriptHelp(BackColor = EditorHeaderBackColor.Steel)]

public class CustomNetworkDebugStartGUI : Behaviour
{
    ...
    CustomNetworkDebugStart _networkDebugStart;
    string _clientCount;
    bool _isMultiplePeerMode;

    Dictionary<CustomNetworkDebugStart.Stage, string> _nicifiedStageNames;
    ...
    protected virtual void Awake()
    {
        ...
        _nicifiedStageNames = ConvertEnumToNicifiedNameLookup<CustomNetworkDebugStart.Stage>("Fusion Status: ");
        _networkDebugStart = EnsureNetworkDebugStartExists();
        _clientCount = _networkDebugStart.AutoClients.ToString();
        ValidateClientCount();
    }
    ...
    protected CustomNetworkDebugStart EnsureNetworkDebugStartExists()
    {
        ...
        if (TryGetBehaviour<CustomNetworkDebugStart>(out var found))
        {
            _networkDebugStart = found;
            return found;
        }

        _networkDebugStart = AddBehaviour<CustomNetworkDebugStart>();
        return _networkDebugStart;
    }
    ...
    private void Update()
    {
        ...
        if (nds.StartMode != CustomNetworkDebugStart.StartModes.UserInterface)
        {
            return;
        }

        var currentstage = nds.CurrentStage;
        if (currentstage != CustomNetworkDebugStart.Stage.Disconnected)
        {
            return;
        }
    ...
    protected virtual void OnGUI()
    {
        ...
        var currentstage = nds.CurrentStage;
        if (nds.AutoHideGUI && currentstage == CustomNetworkDebugStart.Stage.AllConnected)
        {
            return;
        }
        ...
        GUILayout.BeginArea(new Rect(leftBoxMargin, margin, width, Screen.height));
        {
            GUILayout.BeginVertical(GUI.skin.window);
            {
                GUILayout.BeginHorizontal(GUILayout.Height(height));
                {
                    ...
                    if (nds.AutoHideGUI == false && nds.CurrentStage == CustomNetworkDebugStart.Stage.AllConnected)
                    {
    ...
    private void StartHostWithClients(CustomNetworkDebugStart nds)
    {
        int count;
        try
        {
    ...
    private void StartServerWithClients(CustomNetworkDebugStart nds)
    {
        int count;
        try
        {
    ...
    private void StartMultipleClients(CustomNetworkDebugStart nds)
    {
        int count;
        try
        {
    ...
    private void StartMultipleAutoClients(CustomNetworkDebugStart nds)
    {
        int.TryParse(_clientCount, out int count);
        nds.StartMultipleAutoClients(count);
    }

    private void StartMultipleSharedClients(CustomNetworkDebugStart nds)
    {
        int count;
        try
        {
