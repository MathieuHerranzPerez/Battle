...
public class CustomNetworkDebugStart : Behaviour
{

    /// <summary>
    /// Selection for how <see cref="CustomNetworkDebugStart"/> will behave at startup.
    /// </summary>
    ...
    /// <summary>
    /// <see cref="CustomNetworkDebugStartGUI"/> will not render GUI elements while <see cref="CurrentStage"/> == <see cref="Stage.AllConnected"/>.
    /// </summary>
    [InlineHelp]
    ...
    /// If this field is null or invalid, will be set to the current scene when <see cref="CustomNetworkDebugStart"/> runs Awake().
    /// </summary>
    [InlineHelp]
    [ScenePath]
    ...
    /// <summary>
    /// Indicates which step of the startup process <see cref="CustomNetworkDebugStart"/> is currently in.
    /// </summary>
    [InlineHelp]
    [SerializeField]
    [EditorDisabled]
    ...
    {
        if (TryGetComponent<CustomNetworkDebugStartGUI>(out var ndsg) == false)
        {
            ndsg = gameObject.AddComponent<CustomNetworkDebugStartGUI>();
        }
    }
    ...
    protected virtual void Start()
    {
        ...
        if (existingrunner && existingrunner != RunnerPrefab)
        {
            if (existingrunner.State != NetworkRunner.States.Shutdown)
            {
                ...
                var gui = GetComponent<CustomNetworkDebugStartGUI>();
                if (gui)
                {
                    Destroy(gui);
                }
        ...
        {
            if (TryGetComponent<CustomNetworkDebugStartGUI>(out var _) == false)
            {
    ...
    protected IEnumerator StartWithClients(GameMode serverMode, SceneRef sceneRef, int clientCount)
    {
        ...
        {
            Debug.LogWarning($"{nameof(CustomNetworkDebugStart)} can't be a child game object, un-parenting.");
    ...
    protected virtual Task InitializeNetworkRunner(NetworkRunner runner, GameMode gameMode, NetAddress address, SceneRef scene, Action<NetworkRunner> initialized)
    {
        ...
        var pool = runner.GetComponent<INetworkObjectPool>();
